...
#                   Providing the macOS Installer path has been fully tested, now completely stable.
...
#                   No longer gives you "Permission Denied" when launching the script for the first time.
#                   
...
if [[ "$os_version" == 10.6 ]]; then
	...
	if [[ "$prompt" == '1' ]]; then
		...
		if [[ "$AppPath" == '' ]]; then
			...
			if [[ "$AppPath" == *'Mavericks.app'* || "$AppPath" == *'Yosemite.app'*|| "$AppPath" == *'Capitan.app'* || "$AppPath" == *'macOS Sierra.app'* ]]; then
				clear
				echo -e "${APP}${BOLD}                               macOS Creator V3.0"
				echo -e "$LINES"
				echo -e "${RESET}${TITLE}${BOLD}Manually provide the macOS Installer and drive path:"
				...
				if [[ "$DrivePath" == '' ]]; then
					...
					if [[ "$prompt" == 'y' ]]; then
						...
						echo -e "${APP}${BOLD}                               macOS Creator V3.0"
						echo -e "$LINES"
						echo -e "${RESET}${TITLE}Creating the installer...${BODY}"
